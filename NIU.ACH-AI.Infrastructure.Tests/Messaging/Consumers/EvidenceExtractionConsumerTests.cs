...
public class EvidenceExtractionConsumerTests
{
    ...
    private readonly Mock<ILogger<EvidenceExtractionConsumer>> _mockLogger;
    private readonly EvidenceExtractionConsumer _consumer;

    public EvidenceExtractionConsumerTests()
    {
        ...
        _mockLogger = new Mock<ILogger<EvidenceExtractionConsumer>>();

        _consumer = new EvidenceExtractionConsumer(
    ...
    {
        // Arrange
        var contextMock = new Mock<ConsumeContext<IEvidenceExtractionRequested>>();
        var command = new
        {
        ...
        var messageMock = new Mock<IEvidenceExtractionRequested>();
        ...
        _mockOrchestrationExecutor
            .Setup(x => x.ExecuteAsync(It.IsAny<IOrchestrationFactory<List<Evidence>>>(), It.IsAny<OrchestrationPromptInput>(), It.IsAny<StepExecutionContext>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<Evidence>());

        _mockFactoryProvider
            .Setup(x => x.CreateFactory<List<Evidence>>(It.IsAny<ACHStepConfiguration>()))
            .Returns(new Mock<IOrchestrationFactory<List<Evidence>>>().Object);

        _mockWorkflowResultPersistence
            .Setup(x => x.SaveEvidenceAsync(It.IsAny<Guid>(), It.IsAny<List<Evidence>>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<Evidence>());
        ...
        _mockWorkflowResultPersistence.Verify(x => x.SaveEvidenceAsync(
            persistedStepId, 
            It.IsAny<List<Evidence>>(), 
            It.IsAny<CancellationToken>()), Times.Once);
        ...
        // contextMock.Verify(x => x.Publish<IEvidenceExtractionResult>(It.IsAny<IEvidenceExtractionResult>()), Times.Once);
        // contextMock.Verify(x => x.RespondAsync<IEvidenceExtractionResult>(It.IsAny<IEvidenceExtractionResult>()), Times.Once);
    }
}
